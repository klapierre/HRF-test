...
#test on just mt carolina data
mtcaCover <- subset(nutnetRelCover, site_code=='mtca.au')%>%
  #transpose
  spread(key=Taxon, value=rel_cover, fill=0)

#create a dataframe with plot numbers
plotList <- data.frame(plot=unique(mtcaCover$plot))

#create an empty dataframe to collect dissimilarity values
mtcaBC <- data.frame(row.names=1)

#for each plot, calculate bray-curtis dissimilarity across all years
for(i in 1:length(plotList$plot)) {
  
  #get site cover data
  dataset=mtcaCover[mtcaCover$plot==as.character(plotList$plot[i]),]
  
  #calculate bray-curtis dissimilarity
  BC <- as.data.frame(as.matrix(vegdist(dataset[,6:68], method='bray', diag=F, upper=T)))
  ...
  #paste all data into dataframe made for the analysis
  mtcaBC <- rbind(dissimilarity, mtcaBC)
